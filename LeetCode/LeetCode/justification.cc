...
class Solution {
    ...
    private:
        string connect(const vector<string>& words, int maxWidth, int begin,
                int end, int len, bool is_last) {
            string s;
            int n = end - begin;
            for(int i = 0; i<n; ++i) {
                s += words[begin +i];
...
std::ostream& operator<<(std::ostream& o, std::vector<string> const& v) 
{
...
int main() 
{
    Solution s;
    vector<string> words = {
        "This", "is", "an", "exmaple", "of", "text", "justification."
    };
    std::cout<<s.fullJustify(words, 15)<<std::endl;
    return 0;
}
